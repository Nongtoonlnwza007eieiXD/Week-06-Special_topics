...
1. **Multiple Source Files**: เหตุใดต้องแยก source code เป็นหลายไฟล์?
    แบ่งหน้าที่ของแต่ละโมดูลชัดเจน (Separation of Concerns)
    แก้ไข บำรุงรักษา และทดสอบโค้ดได้ง่าย
    ลดความซับซ้อนของไฟล์เดียวที่มีโค้ดจำนวนมาก
    
    
2. **CMakeLists.txt Management**: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?
    ต้องเพิ่มชื่อไฟล์ .c หรือ .cpp เข้าไปใน SRCS ของคำสั่ง idf_component_register

3. **Header Files**: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?
    ประกาศฟังก์ชัน ตัวแปร โครงสร้าง เพื่อให้ไฟล์อื่นเรียกใช้ได้
    ทำให้สามารถรวมโค้ดจากไฟล์อื่นแบบไม่ต้องรู้รายละเอียดภายใน
    ทำไมต้องมี:
    ช่วยให้โมดูลสามารถเชื่อมโยงกันโดยไม่ต้องซ้ำซ้อน
    เป็นการแยกการประกาศ (Declaration) ออกจากนิยาม (Definition)
    
4. **Include Directories**: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?
ใน CMakeLists.txt ต้องระบุ INCLUDE_DIRS เพื่อ:
ให้ระบบ build รู้ว่าต้องค้นหา header files จากไดเรกทอรีใด
ป้องกันปัญหาไม่พบไฟล์ .h ระหว่างการคอมไพล์
สนับสนุนการจัดโครงสร้างโปรเจกต์ให้เป็นระเบียบ

5. **Git Ignore**: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?
กันไฟล์ที่ไม่จำเป็นหรือไม่ควรอยู่ใน Git เช่น build/, *.o, sdkconfig
ลดขนาด repository
ป้องกันความผิดพลาดจากการ commit ไฟล์ชั่วคราวหรือข้อมูลส่วนตัว

6. **Task Management**: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?
แยกการทำงานของ LED ออกจาก main logic
รองรับการทำงานพร้อมกับโมดูลอื่น (multitasking)
จัดการเวลาเปิด/ปิด LED อย่างแม่นยำโดยไม่บล็อกระบบ

7. **Code Organization**: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?
เพิ่มความชัดเจนในหน้าที่ของแต่ละส่วน
ง่ายต่อการแก้ไขหรืออัปเกรดทีละโมดูล
ส่งเสริมการเขียนโค้ดแบบ reusable
ทดสอบแยกแต่ละส่วนได้ง่ายขึ้น
ทำให้โค้ดดูเป็นระบบและดูแลระยะยาวได้ง่าย
...
- จำนวนไฟล์ source: 2 (main.c, sensor.c)
- ขนาด binary: 196,080  bytes
- การทำงาน: อ่านค่าจาก sensor และพิมพ์ผลลัพธ์ผ่าน serial monitor


**ขั้นตอนที่ 2 (เพิ่ม display.c):**
- จำนวนไฟล์ source: 3 (main.c, sensor.c, display.c)
- ขนาด binary: 207,024 bytes 
- การทำงาน:  แสดงค่าที่อ่านได้จาก sensor บนหน้าจอ 


**ขั้นตอนที่ 3 (เพิ่ม led.c):**
- จำนวนไฟล์ source: 4 (main.c, sensor.c, display.c, led.c)
- ขนาด binary:214,208 bytes 
- การทำงาน: ควบคุมไฟ LED ให้เปิดหรือปิดตามค่าจาก sensor 

**สังเกต:**
- การเพิ่มไฟล์ source ส่งผลต่อขนาด binary อย่างไร?
    ขนาดของ binary (.bin) ที่ได้จากการ build เพิ่มขึ้น ตามเนื้อหาของโค้ดในไฟล์นั้น
- LED กะพริบทุกกี่วินาที?
    LED กะพริบทุก 1 วินาที
- แต่ละโมดูลแสดงข้อมูล file และ line อย่างไร?
    หากเปิด log ระดับ debug หรือใช้ ESP_LOG_LEVEL_LOCAL, ข้อมูล log จะแสดง ชื่อไฟล์ (.c) และ หมายเลขบรรทัด (line) 
