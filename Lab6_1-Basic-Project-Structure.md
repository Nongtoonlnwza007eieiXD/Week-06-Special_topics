...
1. **Docker vs Native Setup**: อธิบายข้อดีของการใช้ Docker เปรียบเทียบกับการติดตั้ง ESP-IDF บน host 
  การใช้ Docker ช่วยให้เราพร้อมใช้งานได้ทันที ไม่ต้องติดตั้งอะไรเยอะ แค่มี Docker ก็สามารถรัน ESP-IDF ได้ สะดวกมากโดยเฉพาะถ้าใช้หลายเครื่องหรือทำงานเป็นทีม เพราะทุกคนจะได้ environment เดียวกันหมด ไม่ต้องมากังวลเรื่องเวอร์ชันต่างๆ
ในขณะที่การติดตั้งบนเครื่องจริง (native) อาจจะต้องลงเยอะ และมีโอกาสเจอปัญหาเรื่องเวอร์ชัน หรือ dependency ที่หายไป

2. **Build Process**: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary
ใน Docker เริ่มจากเปิด container แล้วสั่ง source export.sh เพื่อเปิด environment ของ ESP-IDF จากนั้นเข้าไปที่โฟลเดอร์โปรเจกต์ แล้วสั่ง idf.py build ตัวนี้จะอ่านโค้ดของเรา รวบรวมไฟล์ทั้งหมด และแปลงออกมาเป็นไฟล์ binary ที่เอาไปใช้กับบอร์ด ESP32

3. **CMake Files**: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?
ไฟล์ CMakeLists.txt เป็นตัวควบคุมการ build ของโปรเจกต์ บอกว่าเราจะ compile ไฟล์ไหน ใช้อะไรบ้าง และเชื่อมโยงกับ library ไหนบ้าง
Docker มันก็ใช้ไฟล์พวกนี้เหมือนกันทุกประการ แค่ build อยู่ใน container เท่านั้น

4. **Git Ignore**: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?
.gitignore เอาไว้บอก Git ว่าไม่ต้องสนใจไฟล์หรือโฟลเดอร์พิเศษบางอย่าง เช่น โฟลเดอร์ build/ หรือไฟล์ binary ต่างๆ ที่เปลี่ยนแปลงตลอดเวลา การใส่ .gitignore ทำให้โปรเจกต์ใน Git ดูสะอาด ไม่รก และลดปัญหาตอนทำงานร่วมกับคนอื่น

5. **Container Persistence**: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?
ถ้าเราไม่ได้ mount โฟลเดอร์ไว้ ข้อมูลที่อยู่ใน container จะหายหมดเมื่อปิดหรือ restart container เช่น ไฟล์ที่ build แล้ว หรือไฟล์ที่แก้ไว้ใน container โดยตรง แต่ถ้าเราทำ volume mount โฟลเดอร์จากเครื่องเข้ามา เช่น /project -> lab6/ ข้อมูลจะยังอยู่ครบ

6. **Development Workflow**: เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system
การทำงานบน Docker จะได้ environment ที่เหมือนกันทุกเครื่อง ไม่ต้องติดตั้งอะไรเยอะ แค่เปิด container ก็พร้อมทำงาน ส่วน Native จะเชื่อมกับอุปกรณ์ง่ายกว่า แต่ต้องลงเครื่องมือเอง และอาจมีปัญหาเรื่องเวอร์ชันต่างกัน
